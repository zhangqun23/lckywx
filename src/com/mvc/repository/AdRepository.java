...
public interface AdRepository extends JpaRepository<Ad, Integer>{
	//类型为空返回全部广告
	@Query("select a from Ad a where is_delete=1 and ad_state=1")
	public List<Ad> findAlls();
	//返回相应类型广告
	@Query("select t from Ad t where ad_type = :ad_type and is_delete=1 and ad_state=1")
	...
	@Query("select o from Ad o where ad_type = :ad_type and ad_state = :ad_state and open_id = :open_id and is_delete=1")
	public List<Ad> findMyPlaceAd(@Param("ad_type") Integer adType, @Param("ad_state") Integer adState, @Param("open_id") String openId);
	//根据openId查找广告
	@Query("select o from Ad o where open_id = :open_id and is_delete=1")
